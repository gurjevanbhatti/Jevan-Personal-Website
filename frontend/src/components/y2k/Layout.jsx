    ...
    <div className="y2k-root min-h-screen w-full pb-20">
      <div className="h-8 bg-[#e77bb8] text-white flex items-center overflow-hidden border-b-[2px] border-[#4a2b5c]">
        <div className="marquee flex gap-10 whitespace-nowrap font-pixel text-[12px] px-4">
          <span>✦ hi babe ♡ you're on {profile.firstName}'s portfolio</span>
        ...
        <div className="max-w-[1240px] mx-auto grid grid-cols-12 gap-4 items-start">
          <div className="col-span-12 md:col-span-3">
            <NavLink to="/" className="block">
              <div className="relative bg-white border-[2px] border-[#4a2b5c] rounded-2xl px-4 py-3 shadow-[4px_4px_0_0_#4a2b5c] overflow-hidden hover:-translate-y-[2px] transition-transform">
                <div className="absolute -top-4 -right-4 rotate-12"><Butterfly className="w-14 h-14" color="#f1a8d0" /></div>
                <div className="font-bubble text-[36px] leading-[0.9] text-[#e77bb8] drop-shadow-[2px_2px_0_#fff]">gurjevan<span className="text-[#b8a3e3]">.exe</span></div>
                <div className="font-pixel text-[10px] text-[#4a2b5c] mt-1">// personal portfolio · {profile.pronouns}</div>
              </div>
            </NavLink>
          </div>
          ...
          <div className="col-span-12 md:col-span-3">
            <form onSubmit={(e) => { e.preventDefault(); if (q.trim()) navigate("/contact"); }} className="bg-white border-[2px] border-[#4a2b5c] rounded-2xl p-2 shadow-[4px_4px_0_0_#4a2b5c] flex items-center gap-2">
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="say hi to gurjevan..." className="flex-1 h-10 px-3 rounded-lg border-[2px] border-[#4a2b5c] bg-white font-body text-[13px] outline-none focus:ring-2 focus:ring-[#f1a8d0]" />
              <button className="h-10 px-3 rounded-lg bg-[#e77bb8] border-[2px] border-[#4a2b5c] text-white shadow-[2px_2px_0_0_#4a2b5c] hover:bg-[#d76aa9] transition-colors">
                <Search size={16} />
              </button>
